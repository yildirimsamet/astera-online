...
makes a player think _"I wonder what happened"_ after they close it.
...
systems _interact_, never from adding more. If you are about to add complexity to create
...
buildings are _infrastructure_ for this loop, not the loop. If they become the main
...
4. **Watching is silent; probing is loud.** You are never told who is watching you; you
   _are_ told when someone probed you. That asymmetry produces the dread.
...
which decision is authoritative. If the change is correct, update the doc _first_.
...
| The leaderboard is the whole local galaxy                               | Order is rounded Dominion descending, then joined-at and player id; combat refreshes it only when a rounded score changes                                                                                                                                                |
...
| Works disruption is 15 / 5 / 0 minutes, capped 15 minutes ahead | Raids refresh rather than stack; a chain raid can never bury production beyond the current fifteen-minute live loop |
...
| Galaxy chat is season-scoped and server-authored                                                  | Author comes only from auth, reads never cross the caller's season, unread excludes self, and wipe deletes messages before players                                                                                                                                                                                               |
| Visible commander identity comes from `accounts.displayName`                                      | `players.name` is season state, never the public name source; planet names are locations, not people                                                                                                                                                                                    |
...
| A Prospector flies at 330, or 495 with a Derrick, and has its own launch overhead                  | The generated field remains fully reachable at both speeds. Its overhead is a RATIO of `TRAVEL.baseMinutes` (under a quarter) and moves with it — D63 moved both                                                                                                                                                                |
...
| **A revealed identity is a route to its world**                                                    | Signals, Chat and Leaderboard close their sheet, focus the matching public planet and keep its dossier collapsed. A planet id accompanies only an identity the existing projection already reveals (D79).                                                                                                                                              |
| **Chat history scrolls; its composer does not**                                                    | The chat sheet owns a bounded body, the message log is its only scroller, and the textarea/send row remains visible at the foot on portrait screens.                                                                                                                                                                                                    |
| **Chat starts with the commander, never before**                                                   | The pre-account rehearsal renders no Chat launcher and mounts no unread query. Chat appears only in the authenticated season.                                                                                                                                                                                                                           |
...
- Before adding anything: _"which core-loop problem does this solve?"_ No answer → out.
...
printed by an individual single-seed CLI run with the pooled assertions in
`packages/sim/test/season.test.ts`.
...
That was the first suspect for the former two reds, but the obvious version of the hypothesis
was TESTED AND REFUTED: scaling `loginsPerDay` ×4 took that gate from two red to five.
Re-deriving the simulator for real-time pacing is still real work; the current green gate does
not close that modelling gap, so do not guess at it.

**The design's central claim came back.** _The informed archetype tops the ladder on every
seed_ was red from D58 (RAIDER took seed 42) and is green again since D61. Nothing was
...
| **Casual players get farmed** — the 2-logins/day archetype finishes at −10k to −19k Dominion | Open     | The only unresolved _design_ problem. Needs real players, not more simulation             |
...
| File                            | Read it when                                      |
| ------------------------------- | ------------------------------------------------- |
| `docs/product-vision.md`        | You need to know _why_ this game exists           |
