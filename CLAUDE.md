...
- `CARGO_HOLDS` lifts `fleetCargo` AND `transferCargoCapacity` through one `cargoMult` (D181); the two count different rosters, never different ladders, and `tech` is required on both. Clan aid's `clanTransferCargoCapacity` does NOT take it — an open inconsistency, reported not fixed. A hold refusal is validated inside the transaction, never in a pre-transaction preamble.
