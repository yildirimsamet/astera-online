...
- Battle Dominion transfer is the uncapped, linear realised exchange
  ...
  battle grade and wreckage never add score of their own (D2). All three terms use raw physical
  `A + C + D`; D208's `A + 2C + 32D` fleet calibration does not reprice the Dominion journal.
...
- Price ordinary hull combat by `atk * hp / (A + 2C + 32D)²`; each tier targets 1.00 / 1.06 / 1.12 / 1.18 efficiency. The rounded value is the L12 production reference. Nullifier’s premium buys shield-only damage and must reduce its ordinary efficiency. Preserve same-tier equal-budget power. **Specialisation redistributes the PRODUCT, never adds to it** (D195, owner-revised 2026-09-14): `LANCE_ATTACK_HP_RATIO` makes every Lance attack-led, while `BULWARK_PROFILE_RATIO` makes Escort more offensive and Fortress more defensive at the same price. Reciprocal attack/HP factors preserve the unrounded product; integer rounding must retain the existing efficiency/counter bands. See `docs/lance-profile-calibration-2026-09-13.md` for the Lance mirror-annihilation trade-off.
...
- The merchant's NEW-calendar rate is **32 alloy = 16 crystal = 1 deuterium** (D208), stored as unit values `1:2:32`. The split remains `leadStride`-snapped and `rateAnchor` remains the whole-number rail anchor. Historical occurrences and ruleset 5–7 definitions retain their persisted D183 rate. A live season keeps the rate it was dealt — `season restamp` is the only door.
...
- A combat hull's round trip is its CLASS (Skirmisher 15 / Lance 20 / Bulwark 25 min) except where a class holds two shapes: the Escort flies `ESCORT_ROUND_TRIP` 18, the faster Bulwark, and takes D195's smaller hold and larger fuel with it (D207).
...
- **The two build queues have one speed ladder each, and neither reaches into the other** (D198). `yardSpeedMult` shaves what FLIES; `robotSpeedMult` (AI Robots, five rungs to a quarter off) shaves what STANDS — everything ordered into CONSTRUCTION, all fourteen: six buildings, four instruments, four satellites. The rule is the QUEUE, never a list of structures, so no structure is an exception a screen cannot explain. `buildMinutes(cost, core, tech)`, `buildingMinutes(id, level, tech)` and `satelliteMinutes(id, core, tech)` (the Uplink's hand-set 5 minutes, D209) are the only three quotes and `tech` is REQUIRED on all; `profileBuilding().minutes` is authored design work and is never quoted. The discount lands AFTER `BUILD.capMinutes`, deliberately unlike `shipMinutes`, so a capped timer still feels the research.
- **Every combat hull carries a hold, and it stays small** (D195/D208). `COMBAT_HOLD` remains `[40, 85, 180, 380]` by tier, times `referenceRoundTrip / 20`. Cargo is a secondary speed/profile trade rather than part of D208's combat-efficiency promise; no combat tier overlaps the one below it.
- **Dedicated transports stay distinct and improve with tier** (D195b/D208). `SUPPORT_HOLD` is `[1000, 3400, 9500, 26000]`; capacity per `A + 2C + 32D` rises at every rung, and even the least efficient transport carries over ten times as much per value as the best warship. `transport-ladder.test.ts` holds the separation.
...
- **Fuel is a fraction of hull VALUE, tilted by speed, and NO TIER IS EXEMPT** (D195, retiring D153's ladder). `hullFuelMass` = `ceil((A + 2C + 32D) × FUEL.perValue × (FUEL.pivotRoundTrip / referenceRoundTrip))`, floor 1. D153's `tierMass` made the ENTRY hull the most fuel-efficient warship in the game (13.4 power/fuel against tier 3's 8.2), which is an instruction to build nothing else; value already carries power, so pricing against it cannot invert. **`bulk` is ground room and nothing else now** — the Hangar that metered it is gone (D184) and fuel no longer reads it. The opening is protected by measuring the granted tank against the granted FLEET, never by exempting a rung. The ONE hand-set thirst is the Garbage Collector's (`SALVAGE.fuelMass`, 10 per 1k, owner instruction, D200); `fuel.test.ts` holds it to exactly that hull.
...
- A pirate's orbital speed comes off the hull table — `PIRATE.speedMin/Max` are 75% of a Cataclysm's and a Dart's figure divided by `TRAVEL.distanceFactor` (D155/D203). Every Skirmisher outruns every pirate; after D203 even a Citadel outruns the slow end of the band. A target faster than the fleet turns `interceptOrbit`'s earliest meeting into a lap of waiting, not a lead.
...
- Capture pays only on DECISIVE, from the living crew actually encountered (not the pirate's original roster), and always lands; `builtEver` never moves (D133/D150).
...
- **A trade ship and the Intergalactic Convoy are the opposite of a pirate on this one point.** They are announced public events, so the active merchant orbit and active convoy diameter ARE published — but only for the occurrence currently live; the future calendar and route seed never leave the server. Fog hides pre-decision knowledge, not a public live event (D156/D201). A convoy attacker remains an ordinary sensor-zoned contact; an unseen five-second engagement exposes only the public moving target/effect, never the attacker's origin, identity or bearing.
...
- Ruleset 8 deals the public-event calendar at fixed Türkiye windows (D201): Trade Ship 01–03,
  ...
  no calendar RNG. `GALAXY_EVENT_KINDS` remains append-only and `withAsteroidShowerLanes` filters
  strictly by kind.
- D204 raises the Intergalactic Convoy's frozen production cap to **2 hours**, every pirate
  hoard resource by **30%** (`hoardValueMult = 1.82`), and the post-hour-35 isotope-rich share
  to exactly **11/50 = 22%**. Convoy cargo/firepower limits, pirate shares/grade/cargo limits,
  and isotope concentration remain separate and unchanged. D204 moves pirate supply 5% → 6.5%;
  D211's +50% spawn rate moves it again to 9.75% through an appended lane, while admission stays
  on the frozen ×1.4 valuation and every established target/index remains byte-identical.
...
- **Only the CAPITAL's Command Core gates what a commander unlocks** (D209, owner-revised 2026-09-13): colony slots open at capital Core 9 / 12 / 15 (`colonyCoreThresholds`), and research is gated and timed by `researchCoreLevel` whichever world funds it. A captured world's Core never counts.
- **Caretaker worlds are guarded and a capture is not a windfall** (D209): every tier has a template garrison; T2/T3 re-arm guard and dome for free (waiting out an open claim), T1 never re-arms; a settled world's stores are SET to its tier's `captureStock` and the founding charge is spent whole; caretaker (NULL-owner) units are deleted, never handed to the settler.
...
- Dispatch never moves the camera automatically. Fleets, probes, transfers, settlement craft, Death Stars and mining/salvage runs keep the commander's current view; tapping their row in the in-flight sheet is the explicit route to focus them (D210, superseding D153's auto-follow rule).
...
- Mobile Fleet craft sheets do not print legacy Hangar bulk: D184 removed that ceiling. Ground emplacements retain bulk because it still spends ground capacity (D205).
- Strategic crafting is temporarily off behind `FEATURE_FLAGS.STRATEGIC_CRAFTING_ENABLED`: the Death Star forge, anti-strategic battery, Death Star Protocol and Interception Grid are display-none, and both authenticated build routes return `STRATEGIC_UNAVAILABLE` 404. Existing launch/resolution data remains intact (D206).
...
- The public Intergalactic Convoy is the authored exception: 11 ranks × 2 lanes contains all 22 mobile Fleet V2 hulls once, at a convoy-only 2× baseline with 64-unit lanes and size-authored longitudinal gaps (the consecutive T3/T4 gaps get six extra units). Bounded ref-driven drift may make each engine hunt but may never reorder ranks; wakes and drives remain batched/instanced, and the rejected pulse rings stay absent. Four full-length instanced flow veils provide the subtle nose-to-rear wind: their geometry stays anchored while vertex waves and domain-warped fragment noise continuously travel through it, with only shader uniforms changing per frame. Focus uses the final formation length for an exact centred camera range. Corsair, Argosy and Paladin declare native `-X` facing. An identified pirate uses one instanced pick sphere per visible hull, never one box over the empty formation wedges (D202).
...
- **The Asteroid Shower, merchant and Intergalactic Convoy SHIP ON; the economy is measured WITHOUT all three** (owner instruction, 2026-09-12). Shipping-calendar creation is independent from economy measurement. ARR/VFR/progression/season simulations never deal an event calendar or import the shower, trade quote, convoy reward quote or ship roll; `packages/sim/test/economy-scope.test.ts` fails the day any of those surfaces enter the simulator. `tools/economy-design-validation.ts` lists `intergalactic-convoy` in its explicit exclusions. Never switch off a live event merely to keep it out of a balance reading.
