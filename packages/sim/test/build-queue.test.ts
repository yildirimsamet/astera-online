...
describe('the simulator build queue mirror', () => {
  it('runs commander Research independently from Construction and Yard', () => {
    ...
    expect(projectedBuildState(player, world, 'CONSTRUCTION').research)
      .not.toContain('ISOTOPE_SPECTROMETRY');
    expect(projectedBuildState(player, world, 'RESEARCH').research)
      .toContain('ISOTOPE_SPECTROMETRY');
