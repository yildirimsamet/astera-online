/*
 * Existing live worlds predate sensor_epochs. Start their history at deployment:
 * never award a crossing that happened before this build, and never duplicate an
 * epoch if 0042 was applied from an intermediate development build.
 */
WITH "core_levels" AS (
...
WHERE "p"."player_id" IS NOT NULL
	...
	AND NOT EXISTS (
		SELECT 1
		FROM "sensor_epochs" "existing"
		WHERE "existing"."planet_id" = "p"."id" AND "existing"."ends_at" IS NULL
	);
